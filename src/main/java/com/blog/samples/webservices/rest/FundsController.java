...
public class FundsController {
	...
	/**
	 * Gets all funds in database.
	 * 
	 * Example using Hibernate framework
	 *
	 * @return the funds in table funds
	 */
	@RequestMapping(value = "/rest/fundsdb/", method = RequestMethod.GET)
	public ModelAndView getFundsDB() {
		List<Fund> funds = null;

		try {
			funds = fundService.getAllFundsDB();
		} catch (Exception e) {
			String sMessage = "Error getting all funds. [%1$s]";
			return createErrorResponse(String.format(sMessage, e.toString()));
		}

		logger_c.debug("Returning Funds from database: " + funds.toString());
		return new ModelAndView(jsonView_i, DATA_FIELD, funds);
	}
	...
	public ModelAndView createFund(@RequestBody Fund fund_p,
		...
		try {
			createdFund = fundService.createFund(fund_p);
		} catch (Exception e) {
	...
	public ModelAndView updateFund(@RequestBody Fund fund_p, @PathVariable("fundId") String fundId_p,
		...
		try {
			fund = fundService.updateFund(fund_p);
		} catch (Exception e) {
	...
	public ModelAndView removeFund(@PathVariable("fundId") String fundId_p,
		...
		try {
			fundService.deleteFund(fundId_p);
		} catch (Exception e) {
